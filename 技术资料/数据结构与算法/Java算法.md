...
基数排序，桶排序，计数排序
稳定排序：
归并 基数 直接插入 冒泡 ，也就是重复的数，经过排序后，是否前后位置依旧一致
归并，基数，直接插入，冒泡 
基数排序是线性的时间复杂度
其他都不稳定 无论堆排序，还是快排
```
...
抢红包其实不只是个概率问题，是个高并发设计题目，和秒杀差不多

##### 首先是拆红包

##### 预拆包

拆红包一种是预拆包，从队列中获取删除，预算的策略可以避免对共享资源的抢夺，强行把多线程变成单线程，

预拆包的策略在发红包时将金额M的红包拆分成N份，将分配好的结果放入内存队列或者cache，通过incr操作在用户抢红包时分配预算好的红包slot

##### 实时拆包

实时拆包要实时计算，计算的话，就是剩余红包数量+

应对并发写的通常思路是串行化和乐观锁。在用户抢红包时实时拆包计算金额，每抢到一个红包，就cas更新剩余金额和红包个数

##### 高并发读

应对高并发读的通常思路是业务层拦截过滤无效请求，使用有效的缓存。可以使用Cache层decr功能记录请求红包的用户数，当decr到0后就拦截后面的请求直接返回，对DAO层也要增加相应的缓存减少数据库的压力。

用cache层记录当前的请求用户数，超了就拦截请求，cache的写是原子性的，尤其redis还是单线程
